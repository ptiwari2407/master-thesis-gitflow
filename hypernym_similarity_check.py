...
# Number of random words generated on the fly for the algorithm and hence its length should be equal to number of words in hyper_pair
np.random.seed(412)
r_index = np.random.randint(low=0, high= len(random_list), size= len(hyper_pair))
...
not_in_vocab, correct_pred, false_pred = similarity_check_1(hyper_pair, random_list)
...
not_in_vocab, correct_pred, false_pred = similarity_check_1(hyper_pair, random_list, 1)
